    ...
    override fun onCreateView(
        ...
        binding.imageFlashCardSaveWhite.setOnClickListener {
            with(binding){
                ...
                if (currentTripleIndex >= 0 && currentTripleIndex < RightsSingleton.hashMapRights.size) {
                    // Remove the item at the current index from your data structure (e.g., HashMap)
                    val removedTriple = RightsSingleton.hashMapRights.entries.elementAt(currentTripleIndex)
                    RightsSingleton.hashMapRights.remove(removedTriple.key)
